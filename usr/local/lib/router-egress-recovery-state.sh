...
reg_pool_mtime_epoch() {
    ...
    value="$(stat -c %Y "$path" 2>/dev/null | head -n1)"
    printf '%s\n' "$value" | grep -Eq '^[0-9]+$' && { echo "$value"; return 0; }
    value="$(date -r "$path" +%s 2>/dev/null | head -n1)"
    printf '%s\n' "$value" | grep -Eq '^[0-9]+$' && { echo "$value"; return 0; }
    value="$(find "$path" -maxdepth 0 -printf '%T@\n' 2>/dev/null | sed 's/\..*$//' | head -n1)"
    printf '%s\n' "$value" | grep -Eq '^[0-9]+$' && { echo "$value"; return 0; }
    echo 0
}
...
reg_get_state() {
    key="$(reg_key_safe "$1")"
    default="${2:-}"
    reg_init_state >/dev/null 2>&1 || { printf '%s\n' "$default"; return 1; }
    value="$(grep "^${key}=" "$REG_STATE_KV" 2>/dev/null | tail -n1 | sed 's/^[^=]*=//')"
    [ -n "$value" ] && printf '%s\n' "$value" || printf '%s\n' "$default"
}
...
reg_quarantine_endpoint() {
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$(reg_clean_tsv "$ts")" "$(reg_clean_tsv "$utc")" \
        "$(reg_clean_tsv "$egress")" "$(reg_clean_tsv "$iface")" \
        "$(reg_clean_tsv "$endpoint")" "$(reg_clean_tsv "$replacement")" \
        "$(reg_clean_tsv "$reason")" "$(reg_clean_tsv "$pool_path")" \
        "$(reg_clean_tsv "$pool_mtime")" "$(reg_clean_tsv "$source_step")" >>"$REG_QUARANTINE_TSV"
}
...
reg_endpoint_quarantined_for_pool() {
    ...
    [ "$pool_mtime" -gt "$quarantine_ts" ] 2>/dev/null && return 1
    return 0
}
...
reg_counter_get() {
    key="$1"
    reg_init_state >/dev/null 2>&1 || { echo 0; return 1; }
    file="$(reg_counter_file "$key")"
    if [ -e "$file" ]; then
        value="$(tail -n1 "$file" 2>/dev/null | sed 's/[^0-9].*$//')"
...
reg_repair_events_reset() {
    reg_counter_set "$REG_REPAIR_EVENTS_KEY" 0 || return 1
...
reg_selftest() {
    ...
    reg_init_state || return 1
    sleep 1
    reg_quarantine_endpoint "$endpoint" egressX vpnX "$replacement" selftest "$pool" STEP_R15A_SELFTEST || return 1
    reg_endpoint_quarantined_for_pool "$endpoint" "$pool" || return 1
    sleep 1
    printf '\n# refreshed %s\n' "$(reg_now_utc)" >>"$pool"
    touch "$pool" 2>/dev/null || true
    reg_endpoint_quarantined_for_pool "$endpoint" "$pool" && return 1
    reg_set_state mode LOCAL_REPAIR_READY || return 1
    [ "$(reg_get_state mode UNKNOWN)" = "LOCAL_REPAIR_READY" ] || return 1
